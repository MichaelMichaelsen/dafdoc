...
Left join navn						as n 	on v.id = n.cvrenhedsid and n.Vaerdi like ('%'||pNavn||'%' /* filter på Navn*/
Left join telefonnummer				as t 	on v.id = t.cvrenhedsid
Left join adressering 				as ap 	on v.id = ap.cvrenhedsid							/*anvendes ved postadresse*/
Left join adressering 				as ab 	on v.id = ab.cvrenhedsid							/*anvendes ved beliggenhedsadresse*/
...
where 
ap.adresseringanvendelse = 'PostAdresse' 			and /*anvendes ved postadresse*/
ab.adresseringanvendelse = 'Beliggenhedsadresse' 	and /*anvendes ved beliggenhedsadresse*/
...
v.virkningfra = 	(select max(virkningfra) from virksomhed				as vv	where v.id = vv.id 				and vv.virkningfra <= pVirkningstid ) and
e.virkningfra =		(select max(virkningfra) from emailadresse				as ev	where v.id = ev.cvrenhedsid 	and ev.virkningfra <= pVirkningstid ) and
n.virkningfra =		(select max(virkningfra) from navn						as nv	where v.id = nv.cvrenhedsid 	and nv.virkningfra <= pVirkningstid ) and
t.virkningfra =		(select max(virkningfra) from telefonnummer				as tv	where v.id = tv.cvrenhedsid 	and tv.virkningfra <= pVirkningstid ) and
ap.virkningfra =	(select max(virkningfra) from adressering				as apv	where v.id = apv.cvrenhedsid	and apv.virkningfra <= pVirkningstid ) and 								/*anvendes ved postadresse*/
ab.virkningfra =	(select max(virkningfra) from adressering				as abv	where v.id = abv.cvrenhedsid	and abv.cvirkningfra <= pVirkningstid ) and								/*anvendes ved beliggenhedsadresse*/
f.virkningfra  =	(select max(virkningfra) from virksomhedsform			as fv	where v.id = fv.cvrenhedsid 	and fv.virkningfra <= pVirkningstid ) and 								/*anvendes ved virksomhedsform*/
...
k.virkningfra =		(select max(virkningfra) from Kreditoplysninger 		as kv	where v.id = kv.cvrenhedsid		and kv.virkningfra <= pVirkningstid ) and								/*anvendes ved Kreditoplysninger*/
R.virkningfra =		(select max(virkningfra) from reklamebeskyttelse 		as Rv	where v.id = Rv.cvrenhedsid		and Rv.virkningfra <= pVirkningstid ) and 								/*anvendes ved reklamebeskyttet.*/
AD.virkningfra =	(select max(virkningfra) from ansvarligDataleverandoer 	as ADv	where v.id = ADv.cvrenhedsid	and ADv.virkningfra <= pVirkningstid ) and								/*anvendes ved reklamebeskyttet.*/
((pInkluderOphoerteVirksomheder = true and v.virksomhedophoersdato is not null) or (pInkluderOphoerteVirksomheder = false and v.virksomhedophoersdato is null));							/*inkludere opoerte virksomheder*/ 
