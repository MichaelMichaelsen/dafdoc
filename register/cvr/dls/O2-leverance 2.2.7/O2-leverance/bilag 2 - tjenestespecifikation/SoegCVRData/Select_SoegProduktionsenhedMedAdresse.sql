...
CVRE.enhedsType,				/*Virksomhed*/
CVRE.forretningsnoegletype,		/*CVRNummer*/
CVRE.forretningsnoegle,			/*"description": "EAID_564C37C6_7647_4f64_9116_5B273C30D955"*/
...
left join adressering 			as ap 	on P.id = ap.cvrenhedsid and ap.adresse = pAdresse				/*anvendes ved postadresse Search Kreterer*/
left join adressering 			as ab 	on p.id = ab.cvrenhedsid and ab.adresse = pAdresse				/*anvendes ved beliggenhedsadresse Search Kreterer*/
left join branche 				as Hb 	on P.id = Hb.cvrenhedsid 	and hb.sekvens  = 0 				/* anvendes ved Hovbranche*/
left join branche 				as bb1 	on p.id = bb1.cvrenhedsid  	and bb1.sekvens = 1 				/*anvendes ved Bi branche 1*/
left join branche 				as bb2 	on p.id = bb2.cvrenhedsid 	and bb2.sekvens = 2 				/*anvendes ved Bi branche 2*/
left join branche 				as bb3 	on p.id = bb3.cvrenhedsid 	and bb3.sekvens = 3 				/*anvendes ved Bi branche 3*/
left join reklamebeskyttelse 	as R 	on P.id = R.cvrenhedsid  										/*anvendes ved reklamebeskyttet*/
where
...
R.registreringfra 	= (select max(registreringfra) from reklamebeskyttelse 	as Rr  where P.id = Rr.cvrenhedsid		and registreringfra <= pRegistreringstid)) and 						/* anvendes ved reklamebeskyttet*/ 
...
R.virkningfra 	= (select max(virkningfra) from reklamebeskyttelse 	as Rv  where P.id = Rv.cvrenhedsid  and Rv.virkningfra 	<= pVirkningstid );									/* anvendes ved reklamebeskyttet.*/
