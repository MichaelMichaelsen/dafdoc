...
Left join adressering 				as ap 	on v.id = ap.cvrenhedsid
Left join adressering 				as ab 	on v.id = ab.cvrenhedsid
Left join virksomhedsform 			as f 	on v.id = f.cvrenhedsid														/*anvendes ved virksomhedsform*/
left join branche 					as Hb 	on v.id = Hb.cvrenhedsid 	and hb.sekvens  = 0 and hb.Vaerdi = pBranche	/*anvendes ved Hovbranche Search Kreterer*/
left join branche 					as bb1 	on v.id = bb1.cvrenhedsid  	and bb1.sekvens = 1 and bb1.Vaerdi = pBranche	/*anvendes ved Bi branche 1 Search Kriterer*/
left join branche 					as bb2 	on v.id = bb2.cvrenhedsid 	and bb2.sekvens = 2 and bb2.Vaerdi = pBranche	/*anvendes ved Bi branche 2 Search Kriterer*/
left join branche 					as bb3 	on v.id = bb3.cvrenhedsid 	and bb3.sekvens = 3 and bb3.Vaerdi = pBranche	/*anvendes ved Bi branche 3 Search Kriterer*/
...
R.registreringfra = 	(select max(registreringfra) from reklamebeskyttelse  		as Rr 	where v.id = Rr.cvrenhedsid		and registreringfra <= pRegistreringstid) and							/*nvendes ved reklamebeskyttet */
...
AD.virkningfra =	(select max(virkningfra) from ansvarligDataleverandoer 	as ADv	where v.id = ADv.cvrenhedsid	and virkningfra <= pVirkningstid ) and								/*anvendes ved reklamebeskyttet.*/
