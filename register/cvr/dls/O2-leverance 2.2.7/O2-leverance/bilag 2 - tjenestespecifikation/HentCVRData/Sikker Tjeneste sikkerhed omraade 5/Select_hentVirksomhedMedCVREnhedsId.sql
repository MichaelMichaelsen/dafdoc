...
Left join adressering 				as ab 	on v.id = ab.cvrenhedsid and ab.adresseringanvendelse = 'beliggenhedsadresse' /*anvendes ved beliggenhedsadresse*/
Left join virksomhedsform 			as f 	on v.id = f.cvrenhedsid								/*anvendes ved virksomhedsform*/
Left join branche 					as Hb 	on v.id = Hb.cvrenhedsid  and sekvense = 0			/*anvendes ved Hovbranche*/
Left join branche 					as bb1 	on v.id = bb1.cvrenhedsid and sekvense = 1			/*anvendes ved Bi branche 1*/
Left join branche 					as bb2 	on v.id = bb2.cvrenhedsid and sekvense = 2			/*anvendes ved Bi branche 2*/
Left join branche 					as bb3 	on v.id = bb3.cvrenhedsid and sekvense = 3			/*anvendes ved Bi branche 3*/
Left join Kreditoplysninger 		as k 	on v.id = k.cvrenhedsid								/*anvendes ved Kreditoplysninger*/
Left join reklamebeskyttet 			as R 	on v.id = R.cvrenhedsid								/*anvendes ved reklamebeskyttet*/
Left join ansvarligDataleverandoer 	as Ad 	on v.id = AD.cvrenhedsid							/*anvendes ved ansvarligDataleverandoer*/
Left join beskaeftigelse 			as be 	on v.id = Be.cvrenhedsid and pInkluderBeskaeftigelse = true 
...
Left join 	(select pNummer, Vaerdi from Produktionsenhed Pe 
				left join navn np on p.id = np.CVREnhedid
				where
				(pRegistreringstid is not null and 	
				np.registreringfra = (select max(registreringfra) from navn as npr where p.id = npr.cvrenhedsid and registreringfra <= pRegistreringstid)) and
				np.virkningfra = (select max(virkningfra) from navn as npv where v.id = npv.cvrenhedsid and virkningfra >= pVirkningstid)
				
			)						as p 	on v.CPRNummer = P.tilknyttetVirksomhedsCVRNummer
Left join 	(select id,
			...
			where
				...
				FADi.virkningfra = 		(select max(virkningfra) 		from FuldtAnsvarligDeltagerRelation as FADv where CVREnhed.id = FADv.cvrenhedsid and virkningfra >= pVirkningstidwhere)
...
R.registreringfra = 	(select max(registreringfra) from reklamebeskyttet  		as Rr 	where v.id = Rr.cvrenhedsid		and registreringfra <= pRegistreringstid) and -- anvendes ved reklamebeskyttet 
...
R.virkningfra =		(select max(virkningfra) from reklamebeskyttet 			as Rv	where v.id = Rv.cvrenhedsid		and virkningfra <= pVirkningstid ) and 		/*anvendes ved reklamebeskyttet.*/
