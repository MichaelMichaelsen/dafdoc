...
left join adressering 			as ab 	on p.id = ab.cvrenhedsid and ab.adresseringanvendelse = 'beliggenhedsadresse' /*anvendes ved beliggenhedsadresse*/										/*anvendes ved beliggenhedsadresse*/
...
left join beskaeftigelse 		as be 	on P.id = be.cvrenhedsid
where
CVRE.id = pid			/*filter på CVREnhedsid*/
...
R.registreringfra 	= (select max(registreringfra) from reklamebeskyttelse 	as kr  where P.id = R.cvrenhedsid		and registreringfra <= pRegistreringstid)) and 						/* anvendes ved reklamebeskyttet*/ 
