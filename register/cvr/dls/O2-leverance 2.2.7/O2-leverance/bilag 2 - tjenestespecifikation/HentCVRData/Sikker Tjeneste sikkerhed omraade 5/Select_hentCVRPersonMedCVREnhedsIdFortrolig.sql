...
FROM 
CVREnhed as CVRE 
inner join CVRPerson 		as a 	on CVRE.id = A.id
left join navn 				as n 	on A.id = n.cvrenhedsid
left join adressering 		as ap 	on A.id = ap.cvrenhedsid ap.adresseringanvendelse = 'postadresse' /*anvendes ved postadresse*/
left join adressering 		as ab 	on A.id = ab.cvrenhedsid ab.adresseringanvendelse = 'beliggenhedsadresse' /*anvendes ved beliggenhedsadresse*/
where
...
A.registreringfra 	= (select max(registreringfra) from CVRPerson 			as ar  where A.id = ar.id 				and registreringfra <= pRegistreringstid) and
...
A.virkningfra 	= (select max(virkningfra) from CVRPerson 					as pv  where A.id = pv.id 			and pv.virkningfra 	<= pVirkningstid ) and
