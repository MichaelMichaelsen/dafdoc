...
Left join 	(select pNummer, Vaerdi from Produktionsenhed Pe 
				left join navn np on Pe.id = np.CVREnhedid
				where
				...
				np.virkningfra = (select max(virkningfra) from navn as npv where p.id = npv.cvrenhedsid and virkningfra >= pVirkningstid)
